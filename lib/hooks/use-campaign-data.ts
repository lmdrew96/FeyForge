...
// Hook to get encounter history for active campaign only
export function useCampaignEncounters() {
  const activeCampaignId = useCampaignStore((s) => s.activeCampaignId)
  const encounterHistory = useCombatStore((s) => s.encounterHistory)

  return encounterHistory.filter((enc) => enc.campaignId === activeCampaignId)
}
